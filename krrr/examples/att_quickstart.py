"""Quickstart: ATT (partial-parameter) on a synthetic binary-treatment DGP.

Full ATT divides by P(A=1) and is not a Riesz functional — combine the
partial α̂ with a delta-method EIF (Hubbard 2011) downstream. Here we just
fit the partial representer.

Run from the repo root: `.venv/bin/python examples/att_quickstart.py`
"""
...
from krrr import ATT, Gaussian, KernelRieszRegressor, diagnose_kernel
...
def main() -> None:
    ...
    # Closed-form partial ATT representer: a − (1−a) · π(x)/(1−π(x)).
    truth = a - (1 - a) * pi / (1 - pi)
