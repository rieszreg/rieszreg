...
A fitted `RieszBooster` can be persisted two ways:

- **`joblib.dump` / `joblib.load`** — the sklearn idiom. Single-file pickle. Works with `clone`, `GridSearchCV`, `cross_val_predict` after loading.
- **`booster.save(path)` / `RieszBooster.load(path)`** — a directory with the xgboost model in its native UBJSON plus a `metadata.json` sidecar. Cross-language: R's `RieszBooster$save` and `load_riesz_booster` use the same format.
